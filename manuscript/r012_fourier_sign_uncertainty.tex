...
A production C++17 verifier replays the full certificate, and a separately written Python checker independently reconstructs the witness and verifies the tail and radius comparison.
...
A clean checkout needs only the six frozen coefficient files and the released checkers.
...
The checkers do not formalize the Laguerre--Gaussian Fourier identity or the abstract definition-to-bound implication for $A_+(1)$.
Those facts are stated explicitly here and in \texttt{STATEMENT\_AUDIT.md}; they are the natural targets for future proof-assistant formalization.
The release does not claim a Lean proof at present.
...
\end{verbatim}
The first is the complete production proof replay; the second is the independent exact structural/tail/radius replay.
See \texttt{CLAIM.md}, \texttt{STATEMENT\_AUDIT.md}, and \texttt{VERIFICATION.md} for the statement mapping and trust boundary.
...
The public claim rests on the explicit witness, exact certificate checks, and the mathematical dependencies stated above; it does not rely on private model conversations or hidden reasoning traces.
